...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    generate_thumbnail_module(
        &module_dir.join("assets/weapons"),
        "png",
        "weapon_thumbnail",
        "weapon_thumbnails.rs",
    )?;
    generate_thumbnail_module(
        &module_dir.join("assets/items"),
        "webp",
        "item_thumbnail",
        "item_thumbnails.rs",
    )?;
...
fn generate_thumbnail_module(
    ...
    for entry in fs::read_dir(&asset_dir)? {
        ...
        thumbnails.push((source_key.to_string(), path));
    }
    ...
    let output_path = PathBuf::from(env::var("OUT_DIR")?).join(output_filename);
