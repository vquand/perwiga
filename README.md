...
The first executable vertical slice is implemented in Rust. It provides a local SQLite core, a family-aware module registry, a CLI composition root, and the common wiki, notes, folders, checklists, feeds, calendar, and source-backed entity-context persistence workflows. Arknights: Endfield and Genshin Impact are dedicated runtime game modules. Genshin has title-owned, review-first fetch pipelines for characters, NPCs, regions, weapons, skins, artifacts, domains, and a dated event snapshot, plus additive, idempotent SQLite importers for its bundled snapshots.
