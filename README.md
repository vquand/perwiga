...
The first executable vertical slice is implemented in Rust. It provides a local SQLite core, a family-aware module registry, a CLI composition root, and the common wiki, notes, folders, checklists, feeds, and calendar persistence workflows. Arknights: Endfield is the first dedicated game module.
...
The interface supports Endfield module setup, adding and switching between games, module-specific themes and entity schemas, type filtering, multilingual name and alias search, record creation, editing, detail inspection, aliases, and module-owned operator thumbnails with rarity presentation. Titles without a dedicated module use the generic game fallback. Official and automatic Vietnamese translations remain separate throughout the workflow.
...
`crates/perwiga-core` owns the stable contracts, additive SQLite migration, boundary validation, normalized RSS/Atom parsing, and common workflows. Concrete modules under `games/` and `novels/` implement the `LibraryModule` contract. A module may also supply semantic presentation tokens, allowing the shared UI and future desktop shell to adopt a title-specific color theme without title checks in shared code. The CLI and localhost UAT server are composition roots that register concrete modules, preserving the dependency direction from [ADR-002](docs/decisions/ADR-002-games-and-novels-as-library-works.md).

The current Endfield module exposes source-confirmed baseline types for operators, NPCs, regions, weapons, enemies, missions, events, and items. The shared feed capability supports HTTP/HTTPS RSS and Atom refreshes with bounded retries, response limits, and persisted status, but Endfield does not declare that capability until a confirmed title source exists. No unconfirmed Endfield event API, time zone, or automatic title-specific importer is assumed.
